...
function demoPlan(body) {
  ...
  return {
    ...
    stages: [
      {
        name: 'Сабақтың басы', minutes: 7, method: pe ? 'Қауіпсіздік шеңбері' : 'Ой қозғау', workForm: 'Бүкіл сыныппен жұмыс',
      ...
      {
        name: 'Сабақтың ортасы', minutes: 25, method: pe ? 'Көрсет және орында' : 'Қатені тап және түсіндір', workForm: pe ? 'Топтық жұмыс' : 'Жеке және жұптық жұмыс',
      ...
      {
        name: 'Сабақтың соңы', minutes: 8, method: '3–2–1 рефлексиясы', workForm: 'Жеке жұмыс',
...
function normalizePlan(raw, body) {
  ...
  stages[stages.length - 1].minutes = Math.max(1, stages[stages.length - 1].minutes + (40 - total));
  if (stages.reduce((sum, stage) => sum + stage.minutes, 0) !== 40) return fallback;
  return {
